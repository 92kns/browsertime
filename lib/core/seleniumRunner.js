...
export class SeleniumRunner {
   ...
   * @throws {UrlLoadError}
   */
  async loadAndWait(url, pageCompleteCheck, engine, commands) {
    const driver = this.driver;
    ...
    if (this.options.android[1].memoryUsage) {
      log.info("start");
      let memCommand = `ps -p $(pgrep -f org.mozilla.fenix) -o name=,res=`
      let mdata = await commands.android.shell(memCommand);
      console.log(mdata)
    }
    if (this.options.pageCompleteCheckNetworkIdle) {
      ...
      this.options.pageLoadStrategy === 'none'
        ? await delay(this.options.pageCompleteCheckStartWait || 3000)
        : await delay(2000);
